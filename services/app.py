...
from audios.aprocessor import predict_audio_deepfake as apredictor
import os
import shutil
import mimetypes
import time
...
async def upload_file(file: UploadFile = File(...)):
    ...
    if file.filename.endswith(('.jpeg', '.jpg', '.png')):
        ...
        try:
            ...
            prediction, confidence_score = apredictor(file_path)
            result = {
                "audio_name": file.filename,
                "prediction": "fake" if prediction == 1 else "real",
                "confidence": round(confidence_score, 2) * 100 
            }
            return result
        except Exception as e:
            return {"error": str(e)}
